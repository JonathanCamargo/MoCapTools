function [errorTable, markerData, newGapBoolean] = GapMake(markerData, ikXml, varargin)
...
%   In:
%       markerData - marker data (with no gaps) as a struct, table, or file
...
% use evalc to hide all the text that Osim.IK spits out
ikwarnings = evalc('[~, errorTable, ~] = Osim.IK(markerData, ikXml);');
if verboseLevel == 2
    fprintf('%s\n', ikwarnings);
...
[markerNames, frames] = Osim.getFramesToDelete(errorTable, lowThresh, highThresh); %, fpFile);
